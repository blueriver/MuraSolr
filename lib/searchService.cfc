...
<cffunction name="getPublicSearch" returntype="query" access="public" output="false">
	...
	<cfloop query="rs">
		...
		<cfif rsScore.recordcount>
			...
			<cfset querySetCell(rs,"score", DBScore, rs.currentRow)>
		...
		<cfif len(rs.fileID)>
			...
			<cfif rsScore.recordcount>
				<cfset fileScore = (rsScore.score * 1000)>
				<cfif fileScore gt DBScore>
					<cfset querySetCell(rs,"score", fileScore, rs.currentRow)>
					<cfif len(trim(rsScore.context))>
						<cfset querySetCell(rs,"summary","<p>" & rsScore.context & "</p>",rs.currentRow)>
					</cfif>
				</cfif>
